    ...
    withdraw(number: number, amount: number): void {
        ...
        if(foundAccount !== null){
            if (foundAccount.withdraw(amount) === true) {
                console.log(`\nO saque no valor de ${formatCurrency(amount)} na Conta número ${number} foi realizada com sucesso!`);
            }
        }else{
            console.log(Colors.fg.red, '\nConta não encontrada!', Colors.reset);}
    }
    ...
    deposit(number: number, amount: number): void {
        ...
        if(foundAccount !== null){
            foundAccount.deposit(amount)
                console.log(`\nO depósito no valor de ${formatCurrency(amount)} na Conta número ${number} foi realizada com sucesso!`);
    ...
    transfer(sourceNumber: number, targetNumber: number, amount: number): void {
        ...
        if (sourceAccount !== null && targetAccount !== null) {
            if (sourceAccount.withdraw(amount) === true) {
                ...
                console.log(`\nA transferência no valor ${formatCurrency(amount)} da Conta número ${sourceNumber} para a conta número ${targetNumber} foi feita com sucesso!`);
            }
        }
    }
